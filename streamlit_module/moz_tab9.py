...
def add_punctuation_and_transform(punctuation_model,texts, progress_bar, local_model_option):
...
def process_srt_file(punctuation_model,input_file, output_file, progress_bar,local_model_option):
    ...
    processed_texts = add_punctuation_and_transform(punctuation_model,protected_texts, progress_bar,local_model_option)
...
def tab9_main(nlp,punctuation_model,json_file, input_srt_file, deep_punc_progress_bar,segment_progress_bar,spacy_progress_bar,local_model_option,replace_word):
    # プログレスバーを表示
    ...
    process_srt_file(punctuation_model,input_srt_file, temp_srt_file, deep_punc_progress_bar,local_model_option)
    ...
    srt_output_file = sp.process_srt_file(nlp,temp_srt_output_file,output_name1,progress_bar=spacy_progress_bar,replace_word=st.session_state.replace_word)
    output_name2=f'{base_name}_rv_NR.txt'
    output_name3=f'{base_name}_rv_R.txt'
    txt_output_file=os.path.join(temp_dir,output_name2)
    txtR_output_file=os.path.join(temp_dir,output_name3)

    with open(srt_output_file, "r", encoding="utf-8") as file:
        content = file.read()
    ...
    with open(txtR_output_file,"w",encoding="utf-8") as file2:
        file2.write(cleaned_text)
    

    #txt_output_file,txtR_output_file = sp.process_text_file(nlp,temp_txt_output_file,output_name2)
