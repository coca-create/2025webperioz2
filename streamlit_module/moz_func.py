...
def clear_session_state(tab=None,select_clear=None,option=None):#selsect_clearはtab内に複数のクリアボタン,optionは1つのクリアボタンに複数のradiobuttonケースを対応させる.
    ...
    tab1_keys = ['t1_namelist','t1_upload_files','uploaded_t1_files','t1_uploaded_file','t1_translated_text','output_html_tab1', 'output_files_tab1', 'output_file_tab1', 'output_ja_files_tab1', 'df_data_tab1']
    tab1_multi_keys=['radio_select','output_html_tab1_zero','output_files_tab1_zero','output_file_tab1_zero','tab7_df_zero','output_html_tab1_one','output_files_tab1_one','output_file_tab1_one','tab7_df_one','output_html_tab1_two','output_files_tab1_two','output_file_tab1_two','tab7_df_two','t1_translated_text_zero','t1_translated_text_one','t1_translated_text_two','output_ja_files_tab1_zero','df_data_tab1_zero','output_files_tab1_one','df_data_tab1_one','output_files_tab1_two','df_data_tab1_two']
    ...
    tab2_multi_keys=['uploaded_t2_files',
                             't2_upload_files',
                             't2_namelist',
    ...
    tab6_keys = ['tab9_XLSX_FILES','t6_srtnamelist','t6_jsnamelist','t6_jsons','t6_srts','tab9_output_srt_files','tab9_output_txtR_files','tab9_output_txt_files','tab9_files_ready']#'tab9_output_srt_file','tab9_output_txt_file',
    ...
    if tab == '配布字幕の再編':
        for key in (tab1_keys+tab1_multi_keys):
        ...
        for key in (tab2_keys+tab2_multi_keys):
            if key in st.session_state:
                del st.session_state[key]
            st.session_state[key] = None  # Reset value to None
...
def process_file(input_file,replace_word,browseropen=True,filename="sample.html"):
    import pandas as pd
    ...
    R_file_path=os.path.join(temp_dir,f"{basename}_ed_R.txt")
    ...
    for dot_original, dot_replacement in dot_replacements:
        r_dot_original=re.escape(dot_original)
        dot_new_original = rf"\b{r_dot_original}"
        final_texts = re.sub(dot_new_original, dot_replacement, final_texts)

    with open (R_file_path,"w",encoding='utf-8') as f:
        f.write(final_texts.replace(". ",".\n").replace("? ","?\n").replace("[dot]","."))



    t7_excel_file,t7_df=create_excel_from_srt(english_path=output_file,tail="")#0729
    output_html = f"""<head><meta charset="UTF-8"></head><body><pre style="white-space: pre-wrap; overflow-y: auto; height: 150px; word-wrap: break-word; padding: 10px; font-family: inherit; font-size: inherit;">{output}</pre></body>"""
    
    # return output_html, [output_file, docx_file], output_file
    return output_html, [output_file,t7_excel_file,text_file_path,R_file_path],output_file,t7_df
...
def read_file_content(file):
    if file is None:
        return """<div style='color: orange !important; font-family: inherit; text-align: center; 
                display: flex; align-items: flex-start; justify-content: center; height: 150px; padding-top: 40px;'>
                No file uploaded
                </div>"""
...
def display_file_content(file,replace_word=False):
    ...
    if replace_word:
        ...
        for original, replacement in replacements:
            ...
            #content_for_tab=re.sub(new_original,replacement, content_for_tab)
    ...
    if file.endswith('.txt'):
        ...
        if match:
            ...
            if ext == "_NR.txt":
                doc_filename = os.path.join(temp_dir, f"{basename}_NR.docx")
            elif ext == "_R.txt":
                doc_filename = os.path.join(temp_dir, f"{basename}_R.docx")
            ...
            return file_content_for_html, doc_filename
...
def convert_docx_to_srttxt(docx_files):
    ...
    for docx_file in docx_files:
        try:
            ...
            elif clean_name.endswith('_NR (1)'):
                output_filename = clean_name.replace('_NR (1)', f'_{lang_tail}_NR.txt')
            elif clean_name.endswith("_NR"):
                output_filename = clean_name.replace("_NR", f"_{lang_tail}_NR.txt")
            elif clean_name.endswith('_R (1)'):
                output_filename = clean_name.replace('_R (1)', f'_{lang_tail}_R.txt')
            elif clean_name.endswith("_R"):
                output_filename = clean_name.replace("_R", f"_{lang_tail}_R.txt")
            else:
    ...
    if st.session_state.ja_split:
        split_files=t8.process_files(output_files,zip="NOZIP")
      
    else:
        split_files=[]

    all_files=output_files+split_files

    if len(all_files) > 2:
        timestamp_patch = datetime.now().strftime("%Y%m%d%H%M%S")
        temp_dir = os.path.join(tempfile.gettempdir(), f"session_{st.session_state.user_id}",f"{timestamp_patch}")
        os.makedirs(temp_dir, exist_ok=True)
        zip_filename = os.path.join(temp_dir, f"converted_from_officefiles_{lang_tail}.zip")
...
def process_doc_files(files,replace_word=False):
    ...
    for file in files:
        ...
        if ext == ".srt":
            ...
            doc_filename = f"{basename}_NR.docx"
        elif ext == "_R.txt":
            doc_filename = f"{basename}_R.docx"
