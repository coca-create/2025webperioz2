...
def process_text(nlp,text):
    ...
    text = text.lower()
    text = re.sub(r'(?<=[a-zA-Z0-9])-(?=[a-zA-Z0-9])', '[high]', text)
    # 文を解析
    doc = nlp(text)
    result = []
...
def process_srt_file(nlp,file_path,output_name,progress_bar=None,replace_word=False):
    ...
    for i, line in enumerate(lines):
        ...
        # それ以外はテキストとして処理
            ...
            processed_text = process_text(nlp,line.strip())
            processed_text = processed_text.replace("...","[three_dots]").replace("..",".")
            processed_text = processed_text.replace("[three_dots]","...").replace(":","")
            processed_text = re.sub(r"(?<=\w)- (?=\w)|(?<=\w) -(?=\w)", " ", processed_text)#spacy
...
def process_text_file(nlp,input_file, output_name,replace_word=False):
    if replace_word==True:
    ...
    outputR_file = output_file.replace("_NR","_R")
    processed_textR = processed_text
    ...
    for dot_original, dot_replacement in dot_replacements:
        ...
        processed_textR = re.sub(dot_new_original, dot_replacement,processed_textR)  
    
    processed_textR=processed_textR.replace(". ",".\n").replace("? ","?\n").replace("[dot]",".")

    with open(outputR_file,"w",encoding="utf-8") as f:
        f.write(processed_textR)

    return output_file,outputR_file
